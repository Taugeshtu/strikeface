...
// Modifications:
//   2026-09-03: Wrapped calloc and memcpy in unsafe blocks for Rust 2024 edition compliance.
...
unsafe fn to_cstr(mut s: String) -> *mut c_char {
    let a = unsafe { calloc(1, s.len() + 1).cast::<c_char>() };
    assert!(!a.is_null(), "unable to allocate C string");
    unsafe { memcpy(a.cast::<c_void>(), s.as_ptr().cast::<c_void>(), s.len()) };
    s.scramble();
    a
}
